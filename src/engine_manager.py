...
class EngineManager:
    ...
    def _robust_ping(self) -> bool:
        """
        Robustly pings the engine to ensure it's ready, handling engines
        that are slow to initialize after the 'uciok' handshake.
        """
        if not self.engine:
            return False

        try:
            self.engine.protocol.send_line("isready")
            time.sleep(0.5)
            # No timeout for compatibility with older python-chess versions
            self.engine.ping()
            return True
        except (chess.engine.EngineError, chess.engine.EngineTerminatedError, TypeError):
            # Primary ping failed. Attempting "Desperation Kick".
            # Catch TypeError for robustness across versions.
            try:
                # This sends 'isready' and waits for 'readyok' using default timeout
                time.sleep(5.0)  # Explicit wait as requested
