...
class UCIConfigDialog(QDialog):
    ...
    def __init__(self, engine_path: Path, parent=None):
        ...
        self._is_enforcing_rules = False  # Flag to prevent recursion
    ...
    def _load_engine(self) -> None:
        ...
        engine = None
        try:
            # Use python-chess to handle the UCI handshake, which is more robust
            engine = chess.engine.SimpleEngine.popen_uci(str(self.engine_path))
            self.engine_name = engine.id.get("name", self.engine_path.stem)

            # The engine object has the options readily available
            for option in engine.options.values():
                # Reconstruct the option line to reuse our existing parsing logic.
                # This is a bit of a hack, but it centralizes the parsing.
                line = f"option name {option.name} type {option.type}"
                if option.default is not None:
                    # Handle boolean defaults correctly
                    if isinstance(option.default, bool):
                        line += f" default {'true' if option.default else 'false'}"
                    else:
                        line += f" default {option.default}"
                if option.min is not None:
                    line += f" min {option.min}"
                if option.max is not None:
                    line += f" max {option.max}"
                if option.type == "combo":
                    for var in option.var:
                        line += f" var {var}"
                self._parse_option_line(line)

        except (chess.engine.EngineError, chess.engine.EngineTerminatedError, FileNotFoundError, PermissionError) as e:
            QMessageBox.critical(
                self,
                "Engine Load Error",
                f"Could not load or communicate with engine:\n{self.engine_path}\n\n"
                f"Error: {type(e).__name__}: {e}"
            )
        finally:
            # Ensure the temporary engine process is closed
            if engine:
                try:
                    engine.quit()
                except (chess.engine.EngineError, chess.engine.EngineTerminatedError):
                    # Engine may have already died, which is fine.
                    pass
    ...
    def _create_widget_for_option(self, name: str, opt: dict) -> QWidget | None:
        ...
        if opt_type == 'spin':
            ...
            # Connect to rule enforcer
            widget.valueChanged.connect(self._enforce_larry_rules)

            # Special handling for "Threads"
            if name == "Threads":
                def update_threads_warning(value):
                    if value > 8:
                        widget.setStyleSheet("background-color: #ffdddd;")
                        widget.setToolTip("Warning: High thread count with multiple workers may freeze your PC.")
                    else:
                        widget.setStyleSheet("")
                        widget.setToolTip("")
                
                widget.valueChanged.connect(update_threads_warning)
                update_threads_warning(widget.value())  # Initial check
            ...
            widget.stateChanged.connect(self._enforce_larry_rules)
            return widget
            ...
            widget.currentTextChanged.connect(self._enforce_larry_rules)
            return widget
    ...
    def _enforce_larry_rules(self, changed_option=None):
        """
        Enforce Komodo Dragon's mutually exclusive "Larry's Rules".
        Disables incompatible settings to prevent engine crashes.
        """
        if self._is_enforcing_rules:
            return
        
        self._is_enforcing_rules = True
        
        try:
            widgets = {
                "mcts": self._option_widgets.get("Use MCTS"),
                "arma": self._option_widgets.get("Armageddon"),
                "pers": self._option_widgets.get("Personality")
            }
            if not all(widgets.values()):
                return

            # Read current state
            mcts_on = widgets["mcts"].isChecked()
            arma_text = widgets["arma"].currentText()
            pers_text = widgets["pers"].currentText()

            # Rule 3 (Armageddon Priority): If Armageddon is active, it overrides others.
            if arma_text != "Off":
                if mcts_on:
                    widgets["mcts"].setChecked(False)
                if pers_text != "Default":
                    widgets["pers"].setCurrentText("Default")
            # Rule 1 & 2 (MCTS/Personality Priority): If MCTS or Personality are active, they disable Armageddon.
            elif mcts_on or pers_text != "Default":
                if arma_text != "Off":
                    widgets["arma"].setCurrentText("Off")
                if widgets["arma"].isEnabled():
                    widgets["arma"].setEnabled(False)
                    tooltip_parts = []
                    if mcts_on:
                        tooltip_parts.append("'Use MCTS' is active")
                    if pers_text != "Default":
                        tooltip_parts.append("'Personality' is not 'Default'")
                    widgets["arma"].setToolTip(f"Armageddon is disabled because {', and '.join(tooltip_parts)}.")
            # Base case: All conflicting options are off, ensure Armageddon is enabled.
            else:
                if not widgets["arma"].isEnabled():
                    widgets["arma"].setEnabled(True)
                    widgets["arma"].setToolTip("")

        finally:
            self._is_enforcing_rules = False
    ...
    def _reset_defaults(self):
        ...
        # After resetting, re-apply the rules
        self._enforce_larry_rules()
